...
router.post('/create/product', checkRole('admin'), createProductDto, (req, res) => {
...
router.put('/edit/:id', checkRole('admin'), updateProductDto, (req, res) => {
...
router.patch('/update/:id', checkRole('admin'), updateProductDto, (req, res) => {
  const id = parseInt(req.params.id);
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({
